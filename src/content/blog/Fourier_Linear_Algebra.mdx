...
$$L^2$$ is a little different. It is the collection of all functions, f(x), that take a real number, $$a < x < b$$, and return a complex number, and which fulfill the following property:
...
## The Inner Product
Okay I lied. It's called the inner product. The dot product is simply the specific term for the inner product for the $$ℝ^n$$ vector spaces. So let's first talk about them. The generalized dot product equation is:
$$
...
$$
I will now define the inner product for the $$L^2$$ vector space (the weird one that is a collection of functions), but to be completely honest, I don't entirely know where it came from. *HOWEVER*, with that said, it is an awfully useful definition, and it is that from which the fourier series falls out of!
...
$$
\begin{align*}
s(t) &= a_0 + \sum_{k=1}^\infty a_k \textrm{cos}\left(\frac{2\pi}{T} k t\right) + \sum_{k=1}^\infty b_k \textrm{sin}\left(\frac{2\pi}{T} k t\right),
\newline
a_0 &= \frac{1}{T}\int_{t_0}^{t_0+T}s(t)dt
\newline
a_k &= \frac{2}{T}\int_{t_0}^{t_0+T}s(t)\textrm{cos}\left(\frac{2\pi}{T} kt\right) dt
\newline
b_k &= \frac{2}{T}\int_{t_0}^{t_0+T}s(t)\textrm{sin}\left(\frac{2\pi}{T} kt\right) dt
\end{align*}
$$
...
\end{align*}
$$
Now, consider the limiting behaviour of the right hand side as we let $T \rightarrow \infty$. This means our integral is over infinite time, allowing our signal to be infinite in time. It also implies that $\Delta w$ becomes infinitely small, and so by calculus convention, we're going to rename it $dw$. We're then going to immediately get rid of it by defining a new continuous variable $w = kdw$.
...
$$
The sum is the definition of the [Riemann Integral](https://en.wikipedia.org/wiki/Riemann_integral), so substituting the sum for an integral yields:
$$
...
Since Fourier Analysis is basically just projections of functions in the $L^2$ vectors space, why did we choose to use sines and cosines (or complex exponentials), and not some other infinite family of functions? For one, there is the whole physical intuition in terms of things like sound being a sum of pure tones, but there is also a good mathematical reason. The set of sines and cosines are naturally orthogonal for any choice of region $[t_0,t_1]$. This is not at all true for most other sets of basis functions you might choose, like the set of powers. Consider the set of functions in $L^2[t_0,t_1]$:
$$
...
\end{align*}
$$
It should be fairly obvious that the above is, in fact, not equal to zero for every choice of $[t_0,t_1]$ and for every choice of $n \ne m$. (though it does hold for some, for example $\langle t^m, t^n \rangle=0$ on $[-1,1]$ when n+m is odd. The important point is that it does not hold for all choices.)
