...
$$
For all values of $$k \in [1,2,3....]$$, with $w=\frac{2\pi}{t_1 - t_0}$, such that sin$(kwt)$,cos$(kwt)$ will always complete a whole number of cycles in the range $[t_0,t_1]$, form an orthonormal basis of $$L^2[t_0,t_1]$$:
...
Firstly, to simplify the math, we will be working in $L^2[0,1]$, but the following still applies without loss of generality. This means our basis functions become:
...
\end{align*}
$$
If you're at all familiar with the fourier series, you've probably seen it written on this form. However, we can very easily switch between the two sets of basis. To go from the complex exponentials to the set of sines and cosines, you can use the following formulas:
$$
\begin{align*}
a_0 &= c_0
\newline
