...
So I just gotta reverse the order of the switches, seems easy: ```vhdl SW : in std_logic_vector(17
downto 0); ``` Secondly, it seems like the segments are inverted? Just like with
...
As always, gotta start with importing ieee, declaring what packages we want from it, and then making an entity. This is exactly the same as the other modelling schemes, however, we're gonna be a lil cheeky and turn those 7 HEX std_logic_vectors into one [array of std_logic_vectors](https://stackoverflow.com/questions/56794058/in-vhdl-is-it-possible-to-create-an-array-of-std-logic-vector-without-using-a-t):
...
entity counter is
    port (CLK : in std_logic;
            HEXS : out std_logic_aoa (0 to 7) (0 to 6));
end counter;
```
...
Here I have hardcoded it such that the counter will tick every 1 million clock cycles, giving a frequency of 50Hz. Later I want to be able to set the counting frequency via the switches on the board. However, for now, we also need a process to actually display CNT:
