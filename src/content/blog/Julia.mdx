...
Julia also has support for [multiple dispatch](https://en.wikipedia.org/wiki/Multiple_dispatch), which allows you to define the same function for several different sets of inputs! This also means you can extend existing functions and operators to new types, such as defining plus between two strings:
